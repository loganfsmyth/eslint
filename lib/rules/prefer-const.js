...
module.exports = {
    ...
    create: function(context) {
        ...
        // 'true' if it should error if any 'let'-destructured value should be const
        ...
        function report(node) {
            context.report({
                ...
                fix: function(fixer) {
                    var n = node.parent;
                    for (; PATTERN_TYPE.test(n.type); n = n.parent) continue;
                    if (n === node.parent){
                        if (n === n.parent.id) n = n.parent;
                    }
                    ...
                    if (fixedMap.has(declar)) return;

                    fixedMap.set(declar, declar);

                    return fixer.replaceTextRange([declar.range[0], declar.range[0] + 3], 'const');
                }
            });
        }
